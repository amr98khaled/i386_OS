...
void AddressResolutionProtocol::RequestMACAddress(uint32_t IP_BE)
{
    ...
    arp.hardwareType = 0x0100; // 0x0001 is the hardware type for ethernet but we have to encode it in Big Endian so it is written 0x0100
    arp.protocol = 0x0008; // 0x0800 is the Network type for IPv4 but we have to encode it in Big Endian so it is written 0x0008
    arp.hardwareAddressSize = 6; // mac
    arp.protocolAddressSize = 4; // ipv4
    arp.command = 0x0100; // 0x0001 is the ARP Request command but we have to encode it in Big Endian so it is written 0x0100
    arp.srcMAC = backend->GetMACAddress();
    arp.srcIP = backend->GetIPAddress();
    arp.dstMAC = 0xFFFFFFFFFFFF; // broadcast
    arp.dstIP = IP_BE;

    this->Send(arp.dstMAC, (uint8_t*)&arp, sizeof(AddressResolutionProtocolMessage)); //call Send() which is defined in EtherFrameHandler because arp is inherited from it
