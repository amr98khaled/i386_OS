...
EtherFrameHandler::EtherFrameHandler(EtherFrameProvider* backend, uint16_t etherType)
{
    this->etherType_BE = ((etherType & 0x00FF) << 8)
                       | ((etherType & 0xFF00) >> 8);
    //BE stands for Big Endian so we are converting ethertype to big endian
...
/*
We don't have to remove the size of the checksum because it was removed in amd_am79c973.cpp :
